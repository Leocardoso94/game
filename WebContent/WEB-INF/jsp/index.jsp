		...
		function finalizaAgora(id) {
			...
			$.get('ActionServlet', {
				...
				if(responseText === "Resposta Correta!"){
					$('#Correcao' + id).html('<h4 class="text-center">Resposta: '+resposta+'</h4>');
						...
						<c:choose>
							<c:when test="${empty tarefa.resolvida}">
								...
								<button id="submit${tarefa.id}" name=""
									...
									class="btn btn-primary pretoebranco center-block">Enviar</button>
								<div id="Correcao${tarefa.id}"></div>
							</c:when>
							<c:otherwise>
								<h4 class="text-center">${tarefa.pergunta}</h4>
								<h4 class="text-center">Resposta: ${tarefa.resposta}</h4>
							</c:otherwise>
						</c:choose>
					</c:when>
					<c:otherwise>
